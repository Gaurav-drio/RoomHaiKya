...
import { Star, Building2, Users, ArrowRight, User, AlertCircle, FileText, MessageCircle } from 'lucide-react';
  ...
  return (
    <DashboardLayout role="tenant" title={`Hi, ${user?.name?.split(' ')[0]}!`}>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <StatCard label="Active Bookings" value={bookings.filter((b) => b.status === 'active').length} icon={Building2} color="green" delay={0} />
        <StatCard label="Total Bookings" value={bookings.length} icon={Star} color="amber" delay={0.1} />
        <StatCard label="Quick Chat" value="Owners" icon={MessageCircle} color="pink" delay={0.2} />
      </div>
      ...
      >
        <div className="relative">
          <h2 className="text-lg font-bold">Find your next room in Jaipur</h2>
          <p className="mt-1 text-sm text-white/80">
            Browse verified PGs & flats, chat with owners, and post your requirements — zero brokerage.
          </p>
          <Link to="/rooms" className="mt-4 inline-flex items-center gap-1 rounded-full bg-white px-5 py-2 text-sm font-semibold text-brand-600">
